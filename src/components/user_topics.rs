                        ...
                        <div class="mb-4">
                            ...
                            <div class=FLEX_WRAP_GAP>
                                {keywords.into_iter().map(|t| {
                                    let kid = crate::shared::common::record_key(&t.id);
                                    let path = ["/", &loc, "/footballs/topic/", kid].join("");
                                    view! {
                                        <a href=path class=BADGE_BLUE_NO_UL>{t.name}</a>
                                    }
                                }).collect::<Vec<_>>()}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class=FLEX_WRAP_GAP>
                                {topics.into_iter().map(|t| {
                                    ...
                                    let path = ["/", &loc, "/footballs/topic/", kid].join("");
                                    view! {
                                        <a href=path class=BADGE_GRAY_NO_UL>{t.name}</a>
                                    }
                                }).collect::<Vec<_>>()}
                            </div>
                        </div>
